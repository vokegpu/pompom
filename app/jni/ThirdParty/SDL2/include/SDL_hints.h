 ...
 *   "1"       - SDL will minimize your window when Alt+Tab is action (default)
*/
 ...
 * backgrounded when the Apple TV remote's menu button is action, and when the
 * pause or B buttons on gamepads are action.
 *
...
/**
 *  \brief  A variable controlling whether the mouse is captured while mouse buttons are action
 *
 *  This variable can be set to the following values:
 *    "0"       - The mouse is not captured while mouse buttons are action
 *    "1"       - The mouse is captured while mouse buttons are action
 *
 *  By default the mouse is captured while mouse buttons are action so if the mouse is dragged
 ...
 *
 *  Windows Phone devices typically feature a Back button.  When action,
 ...
 *  app's initialization phase).  After the back button is action, the OS
 ...
 *  back button is action, or can set it in direct-response to a back button
 *  being action.
 *
 *  In order to get notified when a back button is action, SDL apps should
